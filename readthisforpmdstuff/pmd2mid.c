...
typedef struct channel_info
{
	...
	UINT16 ssgRhyKeyMask;	// SSG / SSG-PCM rhythm (14 bits)
	
	UINT8 opnaRhyKeyMask;
	UINT8 opnaRhyMstVol;
	UINT8 opnaRhyVol[6];
	UINT8 opnaRhyPanOn[6];
	
	UINT8 portaTargetNote;	// target note for portamento
	UINT8 portaLength;		// length of portamento slide
} CHN_INF;


static const UINT8 OPNA_RHYTHM_NOTES[6] = {0x23, 0x41, 0x33, 0x2C, 0x2B, 0x3E};
static const UINT8 SSG_RHYTHM_NOTES[14] =
{
	0x24, 0x28, 0x29, 0x2D, 0x30, 0x25, 0x26,
	0x2A, 0x2E, 0x39, 0x0A, 0x0B, 0x0C, 0x0D,
};
...
UINT8 PMD2Mid(UINT8 fileVer, UINT16 songLen, UINT8* songData)
{
	...
	chnInf.opnaRhyMstVol = 0x3C;
	for (curNote = 0; curNote < 6; curNote ++)
	{
		chnInf.opnaRhyVol[curNote] = 0x0F;
		chnInf.opnaRhyPanOn[curNote] = 0x00;
	}
	for (curTrk = 0; curTrk < trkCnt; curTrk ++)
	{
		...
		chnInf.volOnce = 0xFF;
		chnInf.earlyOff = 0;
		chnInf.ssgRhyKeyMask = 0x0000;
		chnInf.opnaRhyKeyMask = 0x00;
		chnInf.portaTargetNote = 0xFF;
		chnInf.portaLength = 0;
		...
		while(! (chnInf.flags & CHNFLAG_STOP))
		{
			...
			if (chnInf.trkMode == TRKMODE_RHYTHM)	// special rhythm channel handling
			{
				...
				else
				{
					...
					{
						curNote = curCmd;
						curCmd = 0x00;	// assume a note
					}
				}
			}
			
			// Rhythm Mode uses bytes 00..BF for notes
			if (chnInf.trkMode == TRKMODE_RHYTHM && curCmd < 0x80)
			{
				// 00..7F -> rest
				// 80..BF xx -> mask of rhythm sounds
				curCmd = curNote;
				
				// turn off all notes first
				for (curNote = 0; curNote < 14; curNote ++)
				{
					if (chnInf.ssgRhyKeyMask & (1 << curNote))
						WriteEvent(&midFileInf, &MTS, 0x90, SSG_RHYTHM_NOTES[curNote], 0x00);
				}
				
				if (curCmd < 0x80)
				{
					chnInf.ssgRhyKeyMask = 0x0000;	// rest
				}
				else
				{
					chnInf.ssgRhyKeyMask = ((curCmd << 8) | (songData[inPos] << 0)) & 0x3FFF;
					inPos ++;
				}
				
				// turn on all newly started rhythm notes
				for (curNote = 0; curNote < 14; curNote ++)
				{
					if (chnInf.ssgRhyKeyMask & (1 << curNote))
						WriteEvent(&midFileInf, &MTS, 0x90, SSG_RHYTHM_NOTES[curNote], curNoteVol);
				}
				
				curDly = songData[inPos];	inPos ++;
				MTS.curDly += curDly;
			}
			else if (curCmd < 0x80)	// note
			{
				curNote = curCmd & 0x0F;
				if (curNote < 0x0C)
				{
				...
				// Store source note for portamento before updating lastNote
				UINT8 portaSourceNote = lastNote;
				...
				curDly = songData[inPos];	inPos ++;
				if (chnInf.flags & CHNFLAG_PORTA && portaSourceNote != 0xFF && chnInf.portaTargetNote != 0xFF)
				{
					// Generate pitch bend events for portamento
					// Calculate pitch difference in semitones from source note to target
					INT16 pitchDiff = (INT16)chnInf.portaTargetNote - (INT16)portaSourceNote;
					// MIDI pitch bend: 0x2000 = center (no bend)
					// Standard pitch bend sensitivity is 2 semitones, so 1 semitone = 0x2000 / 2 = 0x1000
					INT32 targetBend = 0x2000 + (pitchDiff * 0x1000);
					if (targetBend < 0) targetBend = 0;
					if (targetBend > 0x3FFF) targetBend = 0x3FFF;
					
					// Use portamento length if available, otherwise use note length
					UINT32 portaDly = (chnInf.portaLength > 0) ? chnInf.portaLength : curDly;
					
					// Generate pitch bend events over the portamento duration
					// Start from center (source note), slide to target
					UINT32 steps = (portaDly > 0) ? portaDly : 1;
					if (steps > 32) steps = 32;	// limit steps for reasonable file size
					UINT32 stepDelay = (steps > 1) ? (portaDly / steps) : portaDly;
					UINT32 remDelay = (steps > 1) ? (portaDly % steps) : 0;
					
					// Start at center (current note playing)
					WriteEvent(&midFileInf, &MTS, 0xE0, 0x00, 0x40);	// center = 0x2000
					
					// Generate intermediate pitch bends
					for (UINT32 i = 1; i < steps; i++)
					{
						INT32 currentBend = 0x2000 + ((targetBend - 0x2000) * i / steps);
						MTS.curDly += stepDelay;
						WriteEvent(&midFileInf, &MTS, 0xE0, (currentBend >> 0) & 0x7F, (currentBend >> 7) & 0x7F);
					}
					// Final pitch bend to target
					MTS.curDly += stepDelay + remDelay;
					WriteEvent(&midFileInf, &MTS, 0xE0, (targetBend >> 0) & 0x7F, (targetBend >> 7) & 0x7F);
					// Reset to center after portamento completes
					MTS.curDly = 0;
					WriteEvent(&midFileInf, &MTS, 0xE0, 0x00, 0x40);	// reset to center
					
					// Apply remaining delay if portamento length was shorter than note length
					if (portaDly < curDly)
						MTS.curDly += (curDly - portaDly);
				}
				else if (chnInf.earlyOff && songData[inPos] != 0xFB && songData[inPos] != 0xC1)
				{
			...
			else
			{
				...
				case 0xF5:	// set Transposition
					...
					break;
				case 0xB2:	// set secondary transposition
					inPos += 0x01;	// skip signed byte (secondary transposition, not used in MIDI)
					break;
				...
				case 0xEB:	// OPNA Rhythm Key On
					...
					if (tempByt & 0x80)
					{
						...
						for (curNote = 0; curNote < 6; curNote ++)
						{
							if ((chnInf.opnaRhyKeyMask & tempByt) & (1 << curNote))
							{
								chnInf.opnaRhyKeyMask &= ~(1 << curNote);
					...
					else
					{
						...
						for (curNote = 0; curNote < 6; curNote ++)
						{
							...
							{
								if (chnInf.opnaRhyKeyMask & (1 << curNote))
									WriteEvent(&midFileInf, &MTS, 0x90, OPNA_RHYTHM_NOTES[curNote], 0x00);
								else
									chnInf.opnaRhyKeyMask |= (1 << curNote);
								tempVol = PMDVol2Mid(TRKMODE_RHY_CHN, chnInf.opnaRhyVol[curNote], chnInf.opnaRhyPanOn[curNote]);
				...
				case 0xEA:	// set OPNA Rhythm Volume
					tempByt = songData[inPos];	inPos ++;
					curNote = (tempByt >> 5) - 1;
					chnInf.opnaRhyVol[curNote] = tempByt & 0x1F;
					break;
				case 0xE9:	// set OPNA Rhythm Panning
					tempByt = songData[inPos];	inPos ++;
					curNote = (tempByt >> 5) - 1;
					tempByt = PanBits2MidiPan(tempByt & 0x03);
					chnInf.opnaRhyPanOn[curNote] = (tempByt == 0x40) ? 0x00 : 0x01;
					{
						UINT8 midChnBak = MTS.midChn;
						MTS.midChn = 0x09;
				...
				case 0xE8:	// set OPNA Rhythm Master Volume
					chnInf.opnaRhyMstVol = songData[inPos];	inPos ++;
					{
						UINT8 midChnBak = MTS.midChn;
						MTS.midChn = 0x09;
						tempVol = PMDVol2Mid(TRKMODE_RHY_MST, chnInf.opnaRhyMstVol, 0x00);
				...
				case 0xE6:	// add to OPNA Rhythm Master Volume
					chnInf.opnaRhyMstVol += songData[inPos];	inPos ++;
					if (chnInf.opnaRhyMstVol >= 0x80)
						chnInf.opnaRhyMstVol = 0x00;
					else if (chnInf.opnaRhyMstVol > 0x3F)
						chnInf.opnaRhyMstVol = 0x3F;
					{
						UINT8 midChnBak = MTS.midChn;
						MTS.midChn = 0x09;
						tempVol = PMDVol2Mid(TRKMODE_RHY_MST, chnInf.opnaRhyMstVol, 0x00);
				...
				case 0xE5:	// add to OPNA Rhythm Volume
					tempByt = songData[inPos];	inPos ++;
					curNote = (tempByt >> 5) - 1;
					chnInf.opnaRhyVol[curNote] += songData[inPos];	inPos ++;
					if (chnInf.opnaRhyVol[curNote] & 0x80)
						chnInf.opnaRhyVol[curNote] = 0x00;
					else if (chnInf.opnaRhyVol[curNote] > 0x1F)
						chnInf.opnaRhyVol[curNote] = 0x1F;
					break;
				case 0xE3:	// Volume Up
					tempByt = songData[inPos];	inPos ++;
					switch(chnInf.trkMode)
				...
				case 0xDD:	// temporary Volume Down
					...
					if (VOL_MODE == 0x00)
						...
						curNoteVol = tempByt;
					break;
				case 0xDA:	// Portamento: DA aa bb ll (source note, target note, length)
					chnInf.flags |= CHNFLAG_PORTA;
					// Read source note (aa) - skip it, we'll use the current playing note
					tempByt = songData[inPos];	inPos ++;
					// Read target note (bb)
					tempByt = songData[inPos];	inPos ++;
					...
					curNote += chnInf.transp + noteMove;
					chnInf.portaTargetNote = curNote;	// store target note for pitch bend
					// Read length (ll) - duration of the portamento slide
					chnInf.portaLength = songData[inPos];	inPos ++;
					if (! (didInitCmds & 0x02))
					{
						didInitCmds |= 0x02;
				...
				case 0xC9:	// set PSG Modulation "Extend Mode"?
					...
					inPos += 0x01;
					break;
				case 0xBB:	// set "Extend Mode", bit 5
					inPos += 0x01;	// skip one byte
					break;
				case 0xC4:	// set Early Key Off Timeout Percentage
					inPos += 0x01;	// skip percentage byte (used with FE command for early key off)
					break;
				...
				case 0xC0:	// MML Part Mask
					...
					if (tempByt < 2)
					{
						// normal part mask (0 = off, 1 = on)
						WriteEvent(&midFileInf, &MTS, 0xB0, 0x70, curCmd & 0x7F);
						WriteEvent(&midFileInf, &MTS, 0xB0, 0x26, tempByt & 0x7F);
					}
					else
					{
						// special commands (volume down settings)
						switch(tempByt)
						{
						case 0xFF:	// FM volume down (set)
							inPos += 0x01;
							break;
						case 0xFE:	// FM volume down (add)
							inPos += 0x01;	// signed byte
							break;
						case 0xFD:	// SSG volume down (set)
							inPos += 0x01;
							break;
						case 0xFC:	// SSG volume down (add)
							inPos += 0x01;	// signed byte
							break;
						case 0xFB:	// PCM volume down (set)
							inPos += 0x01;
							break;
						case 0xFA:	// PCM volume down (add)
							inPos += 0x01;	// signed byte
							break;
						case 0xF9:	// Rhythm volume down (set)
							inPos += 0x01;
							break;
						case 0xF8:	// Rhythm volume down (add)
							inPos += 0x01;	// signed byte
							break;
						case 0xF7:	// PCM86 volume
							inPos += 0x01;
							break;
						case 0xF6:	// PPZ volume down (set)
							inPos += 0x01;
							break;
						case 0xF5:	// PPZ volume down (add)
							inPos += 0x01;	// signed byte
							break;
						default:
							// unknown special command - treat as end of track
							printf("Unknown C0 special event %02X on track %u at %04X\n", tempByt, trkID, inPos - 0x02);
							chnInf.flags |= CHNFLAG_STOP;
							break;
						}
					}
